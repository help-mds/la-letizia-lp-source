...
### ステップ1: La Letizia Space シーンのみ
- [x] GSAP ScrollTrigger をインポート・登録
- [x] Scene Mode のアーキテクチャ変更: body lock → scrollable sections
- [x] Space シーン用 ScrollTrigger pin + snap (400vh, 5ステップ)
- [x] progress → popup 切り替えロジック (章タイトル→popup1→popup2→popup3→終了)
- [x] ホットスポット glow アニメーション (snap 到達時)
- [x] ポップアップ slide-up/slide-down アニメーション
- [x] スクロールアップで前のポップアップに戻る
- [x] 中間状態なし (snap で離散ステップのみ)
- [x] 黒い空白なし確認
- [x] Selection/Craft/Access/Reservation は既存のまま (後で展開)

### ステップ2: Selection / Craft に展開
- [x] Selection シーン (400vh, 5ステップ)
- [x] Craft シーン (300vh, 4ステップ)

### ステップ3: "?" 直接タップ override
- [x] overrideMode state 追加
- [x] タップで即時表示、× で復帰
- [x] 次 snap point 到達で自動解除
