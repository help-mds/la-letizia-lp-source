...
- [x] touch スワイプダウン = 次シーン、スワイプアップ = 前シーン
- [x] wheel イベント（PC トラックパッド）でも同様に次/前シーン移動
- [x] 横スワイプは無視（ポップアップ操作と干渉しない）
- [x] debounce/cooldown で連続発火防止
- [x] La Letizia で動作確認
- [x] NOA hair で動作確認

## メタリックボタンスタイル + 外枠エンクロージャー
- [x] セクション切り替えボタン（上下矢印）をメタリック/ガラス質スタイルに変更、外側カプセル型エンクロージャー付き
- [x] 共有ボタンをメタリック/ガラス質スタイルに変更、外側円形エンクロージャー付き
- [x] ドットナビゲーションを同様のメタリック/エンクロージャースタイルに変更
- [x] RESERVEボタンをメタリック/ガラス質スタイルに統一（外枠エンクロージャー付き）
- [x] ホットスポット「?」ボタンをメタリック/ガラス質スタイルに変更

## ヘッダーデザイン改善
- [x] ヘッダーをプレミアム感のあるデザインに改善（安っぽさ解消）

## Accessセクション改善
- [x] 住所・営業時間の文字色を修正（白背景に白文字で読めない → 黒/ダーク文字に）
- [x] 店名/コンテンツとドットナビの距離を広げる（上部マージン追加）

## スワイプ制御改善
- [x] ヒーロー後の最初のセクション（Space）に入った直後はスワイプ/ホイールで次に進めないようにする
- [x] セクション切り替えのトランジション（黒画面）中もスワイプ無効にする

## セールスKPI再現の問題修正 & テンプレートスペック追記

### 問題1: スクロールスクラブが動かない
- [x] LP_FACTORY_TEMPLATE_SPEC.md に「スクロールリスナーの注意事項」セクション追加
- [x] PageScrollScrub のスクロールリスナーにフォールバック（#root内スクロール検出）追加

### 問題2: モバイルでヒーローが横向き（上下に黒幕）
- [x] LP_FACTORY_TEMPLATE_SPEC.md に「ヒーロー実装の技術要件」セクション追加（canvas+フレーム方式推奨、video要素の制限事項）
- [x] PageScrollScrub の cover-fit 描画仕様をスペックに明記

### 問題3: UIサイズが小さい
- [x] LP_FACTORY_TEMPLATE_SPEC.md に「UI要素サイズ仕様」セクション追加（デスクトップ/モバイル別）
...
## LP作成フォーム簡素化（/lp-input）

- [x] フォームを3項目のみに簡素化: 業種×グレード選択、店名、GoogleマップURL
- [x] 旧フィールド（エリア、サブタイプ、連絡先、ambiance設定、コピー、メニュー等）を全て削除
- [x] バックエンド（leads.create mutation）は既存のoptionalバリデーションで互換性あり（変更不要）
- [x] フォーム送信→LP生成→URL表示のフロー確認
